<template>
	...
	<div class='chanceCenterWrap' v-cloak>
		<h3 class="pageTitle">
			<span>商机中心</span>
			<em>Business Opportunity Center</em>
		</h3>
			...
			<table class="pageTable" border='0' cellpadding="0" cellspacing="0">
				...
				<tbody>
					<tr v-for='(item,index) in pageArr'>
						...
						<td>{{ item.investmentAmounts }}</td>
						<td>{{ item.shopNum }}</td>
						<td>{{ item.foundingTime }}</td>
						<td>{{ item.province + item.city + item.prefecture }}</td>
						<td>{{ item.compactAgeLimit }}</td>
						<td>{{ item.profession }}</td>
						<td :class='{"":item.isAudit == 0,"yellowTd":item.isAudit == 1,"cyanTd":item.isAudit == 2,"redTd":item.isAudit == 3}'>{{ getState(item.isAudit) }}</td>
						<td>
	...
	export default{
		...
		methods:{
			initPage(){
				...
				$.ajax({
					type:'post',
					url:that.Global.router + '/business/businessCenter',
			...
			checkThis(id){
				...
				that.$confirm('确定提交审核？','提示',{
					...
					$.ajax({
						type:"post",
						url:that.Global.router + '/business/uploadExamine',
						data:{id:id},
						xhrFields:{withCredentials: true},
						success(res){
							console.log(res)
