	...
	export default{
		...
		methods:{
			initPage(){
				...
				$.ajax({
					type:'post',
					url:that.Global.router + '/product/editHomepage',
					...
					success(res){
						...
						if(res.code === "0000"){
							...
							for(var i=0; i<res.result.length; i++){
								that.pageArr.push({id:i,url:res.result[i],show:false})
							}
			...
			savePage(){
				...
				if(that.pageArr.length == 0){
					...
					$.ajax({
						type:'post',
						url:that.Global.router + '/product/updateHomepage',
						data:{url:url},
						xhrFields:{withCredentials: true},
						success(res){
							console.log(res)
